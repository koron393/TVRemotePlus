		...
		if (!empty($EDCB_http_url)){
			...
			if (!$Use_EPGStation == 'true'){
				...
				if (isset($epg->items->eventinfo[0]->service_name)){
					$channel = mb_convert_kana(strval($epg->items->eventinfo[0]->service_name), 'asKV');
				} else {
				...
				if (isset($epg->items->eventinfo[0]->event_name)){
					...
					$program_name = str_replace("\n", "<br>\n", mb_convert_kana(strval($epg->items->eventinfo[0]->event_name), 'asKV'));
				} else {
					if (isset($epg->items->eventinfo[0]->service_name)){
						$program_name = '放送休止';
					} else {
				...
				if (isset($epg->items->eventinfo[0]->event_text)){
					//文字列に変換してさらに半角に変換して改行をbrにする
					$program_info = str_replace("\n", "<br>\n", mb_convert_kana(strval($epg->items->eventinfo[0]->event_text), 'asKV'));
				} else {
					if (isset($epg->items->eventinfo[0]->service_name)){
						$program_info = '放送休止';
					} else {
				...
				if (isset($epg->items->eventinfo[1]->event_name)){
					//文字列に変換してさらに半角に変換して改行をbrにする
					$next_program_name = str_replace("\n", "<br>\n", mb_convert_kana(strval($epg->items->eventinfo[1]->event_name), 'asKV'));
				} else {
				...
				if (isset($epg->items->eventinfo[1]->event_text)){
					//文字列に変換してさらに半角に変換して改行をbrにする
					$next_program_info = str_replace("\n", "<br>\n", mb_convert_kana(strval($epg->items->eventinfo[1]->event_text), 'asKV'));
				} else {
					if (isset($epg->items->eventinfo[1]->service_name)){
				...
				if (isset($epg[0]['programs'][0]['name'])){
					//文字列に変換してさらに半角に変換して改行をbrにする
					$program_name = str_replace("\n", "<br>\n", mb_convert_kana(strval($epg[0]['programs'][0]['name']), 'asKV'));
				} else {
					if (isset($epg[0]['channel']['name'])){
						$program_name = '放送休止';
					} else {
				...
				if (isset($epg[0]['programs'][0]['description'])){
					//文字列に変換してさらに半角に変換して改行をbrにする
					$program_info = str_replace("\n", "<br>\n", mb_convert_kana(strval($epg[0]['programs'][0]['description']), 'asKV'));
				} else {
					if (isset($epg[0]['channel']['name'])){
						$program_info = '放送休止';
					} else {
				...
				if (isset($epg[0]['programs'][1]['name'])){
					//文字列に変換してさらに半角に変換して改行をbrにする
					$next_program_name = str_replace("\n", "<br>\n", mb_convert_kana(strval($epg[0]['programs'][1]['name']), 'asKV'));
				} else {
					if (isset($epg[0]['channel']['name'])){
						$next_program_name = '放送休止';
					} else {
				...
				if (isset($epg[0]['programs'][1]['description'])){
					//文字列に変換してさらに半角に変換して改行をbrにする
					$next_program_info = str_replace("\n", "<br>\n", mb_convert_kana(strval($epg[0]['programs'][1]['description']), 'asKV'));
				} else {
					if (isset($epg[0]['channel']['name'])){
						$next_program_info = '放送休止';
					} else {
